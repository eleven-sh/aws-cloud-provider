#!/bin/bash
# 
# Eleven instance init.
# 
# This is the first script to run during 
# the creation of the instance (via cloud-init).
# 
...
log "\n\n"
log "---- Eleven instance init (start) ----"
log "\n\n"
...
handleExit () {
  ...
  if [[ "${EXIT_CODE}" != 0 ]]; then
    ...
    log "---- Eleven instance init (failed) (exit code ${EXIT_CODE}) ----"
  else
    ...
    log "---- Eleven instance init (success) ----"
  fi
  log "\n\n"
...
# Looking up the instance architecture to
...
if [[ ! -f "${ELEVEN_AGENT_PATH}" ]]; then
  # curl --fail --silent --show-error --location --header "Accept: application/octet-stream" https://api.github.com/repos/eleven-sh/agent/releases/assets/82211381 --output "${ELEVEN_AGENT_PATH}"
  rm --recursive --force "${ELEVEN_AGENT_TMP_ARCHIVE_PATH}"
  curl --fail --silent --show-error --location --header "Accept: application/octet-stream" "https://github.com/eleven-sh/agent/releases/download/v${ELEVEN_AGENT_VERSION}/agent_${ELEVEN_AGENT_VERSION}_linux_${INSTANCE_ARCH}.tar.gz" --output "${ELEVEN_AGENT_TMP_ARCHIVE_PATH}"
